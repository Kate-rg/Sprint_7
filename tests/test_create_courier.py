...
class TestCreateCourier:
    ...
    @allure.title('Создание курьера только с обязательными параметрами')
    @allure.description('Проверяем, что можно создать курьера с указанием только обязательных параметров, в ответе статус 201, тело ответа {"ok":true}')
    def test_courier_without_firstname(self):
        response = requests.post(urls.URL_BASE + urls.URL_CREATE_COURIER, data=TestDataBody.BODY_WITHOUT_FIRSTNAME)
        assert response.status_code == 201 and response.text == '{"ok":true}'

        TestMethodsHelper.delete_courier(TestDataBody.BODY_WITHOUT_FIRSTNAME)
